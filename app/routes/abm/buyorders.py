...
def new():
    userID = User.get_id(current_user)
    providers_list = Providers.getNameList(Providers, userID) # type: list
    SKU_list = SKU.getDict(SKU, userID) # type: dict
    return render_template("logged/abm/buyorder/new.html", name=current_user.name, userID=userID, SKU_list=SKU_list, provider_list=providers_list)
...
def good(IDReceiptOrder, IDBuyOrder):
    return render_template('logged/abm/buyorder/good.html', IDBuyOrder=IDBuyOrder, IDReceiptOrder=IDReceiptOrder)
...
def edit(id):
    ...
    SKU_list = SKU.getDict(SKU, userID)
    providers_list = Providers.getNameList(Providers, userID)
